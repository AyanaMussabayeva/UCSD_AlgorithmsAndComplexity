...
void bfs(const FlowGraph& graph, int s, int t, vector<int>& pathEdges)
{
    queue<int> q;
    q.push(s);

    fill(pathEdges.begin(), pathEdges.end(), -1);
    ...
    while (!q.empty()) {
        ...
        for (auto id : graph.get_ids(cur)) {
            ...
            if (pathEdges[e.to] == -1 && e.capacity > e.flow && e.to != s) {
                pathEdges[e.to] = id;
                q.push(e.to);
            }
        }
...
int max_flow(FlowGraph& graph, int s, int t)
{
    ...
    vector<int> pathEdges(graph.size());
    while (pathEdges[t] != -1){
        bfs(graph, s, t, pathEdges);

        if (pathEdges[t] != -1) {
            int min_flow = numeric_limits<int>::max();
            for (int u = pathEdges[t]; u != -1; u = pathEdges[graph.get_edge(u).from]) {
