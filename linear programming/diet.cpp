...
Position selectPivot(const Matrix& a, vector<bool>& used_rows, vector<bool>& used_columns)
{
    ...
    // select a pivot with the largest absolute value
    long double max = 0.0;
...
int substitute(Matrix& a, Column& b)
{
...
inline void pivotScaling(Matrix& a, Column& b, const Position& pivot_element)
{
...
inline void findUsedPivots(const Position& pivot, vector<bool>& used_rows, vector<bool>& used_columns)
{
...
void processPivot(Matrix& a, Column& b, const Position& pivot_element)
{
    ...
    pivotScaling(a, b, pivot_element);
...
pair<int, Column> solveEquation(Equation equation)
{
    ...
    for (int steps = a.size(); steps; --steps) {
        ...
        findUsedPivots(pivot_element, used_rows, used_columns);
    }

    int code = substitute(a, b);

    return { code, move(b) };
}

vector<vector<int> > findSubsets(const vector<int>& set, int m)
{
...
vector<Column> solveAll(int m, const Matrix& A, const vector<long double>& b)
{
    ...
    auto subsets = findSubsets(nums, m);
    ...
    for (const auto& sub : subsets) {
        ...
        auto code_and_sol = solveEquation(eq);
...
inline void preprocess(int& n, const int m, Matrix& A, vector<long double>& b)
{
    // enusre matrix MxM
    while (n < m) {
        A.emplace_back(m, 0);
...
pair<int, vector<long double> > gaussianEliminationSolving(int n, int m, Matrix A, vector<long double> b, vector<long double> c)
{
    preprocess(n, m, A, b);
    vector<Column> solutions = solveAll(m, A, b);
...
int main()
{
    ...
    pair<int, vector<long double> > ans = gaussianEliminationSolving(n, m, move(A), move(b), move(c));
